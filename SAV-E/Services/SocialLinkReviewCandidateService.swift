...
final class SocialLinkReviewCandidateService {
    ...
    private func analysisMethodAttempts(evidenceText: String, sourceURL: String) -> [String] {
        [
            ...
            "Analysis method: falls back to source-only when readable metadata is missing/blocked, only a creator handle exists, only OCR/headline text exists, only menu/items exist, multiple list places need selection, or no address/map-provider proof is available",
